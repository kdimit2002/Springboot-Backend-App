...
import com.example.webapp.BidNow.Services.DisableUserService;
...
public class NightlyConsistencyScheduler {
    ...
    public NightlyConsistencyScheduler(AdminUserEntityService adminUserEntityService, DisableUserService disableUserService, UserEntityRepository userRepository, FirebaseAuth firebaseAuth, EmailService emailService, DatabaseRetryService databaseRetryService, FirebaseRetryService firebaseRetryService, RoleRepository roleRepository) {
    ...
    private void performCheckFetchingDatabaseUsers() {
        ...
        for (var user : users) {
            try {
                ...
                // An den user iparxi sto database ke oxi sto firebase ke den ine anonymize kaneton
                if (notFound && !user.getAnonymized()) {
                    ...
                    disableUserService.handleDbUserAnonymize(user);
